...
function run_steepest_descent()
    ...
    methods = {@SD_fixed, @SD_fminbnd, @SD_golden_section, @SD_armijo};
    method_names = {"Fixed Step", "Optimal Step (fminbnd)", "Golden Section", "Armijo"};
    
    for i = 1:size(start_points, 2)
        plot_results_for_start(eval_func, gradient_func, methods, method_names, start_points(:, i), e, gamma, a, b, l, i);
    end
end

function plot_results_for_start(f, grad, methods, method_names, start_point, epsilon, gamma, a, b, l, figure_index)
    figure('Name', sprintf('Results for Starting Point (%.2f, %.2f)', start_point(1), start_point(2)));
    ...
    for j = 1:num_methods
        meth = methods{j};
        if j == 1 
            [xk, n, f_values] = meth(f, grad, epsilon, gamma, start_point);
        elseif j == 2 
            [xk, n, f_values] = meth(f, grad, epsilon, b, start_point);
        elseif j == 3
            [xk, n, f_values] = meth(f, grad, epsilon, a, b, l, start_point);
        else 
            [xk, n, f_values] = meth(f, grad, epsilon, gamma, start_point);
        end
...
function [xk, n, f_values] = SD_fminbnd(f, f_grad, epsilon, b, x0)
    xk = x0;
    k = 1;
    f_values = f(xk(1), xk(2));
    while norm(f_grad(xk(1, k), xk(2, k))) > epsilon
...
function [xk, n, f_values] = SD_golden_section(f, f_grad, epsilon, a, b, l, x0)
    xk = x0;
    k = 1;
    f_values = f(xk(1), xk(2));
    while norm(f_grad(xk(1, k), xk(2, k))) > epsilon
...
function [xk, n, f_values] = SD_armijo(f, f_grad, epsilon, gamma, x0)
    alpha = 2e-3;
    beta = 0.5;
    xk = x0;
    k = 1;
